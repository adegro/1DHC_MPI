...
static double* my_chp;
static double* buddy_chp;
static double* T;

/*    x0=0.0;
    xn=100.0;
    number of procs = 4
    based on these values the size of T matrix is on each proc is
*/
static int     npp=250;
static int     Tsize=254;
static double  h=0.1;
static double  t0=0.0;
static double  tn=3.0;
static double  temp1=0.0;
static double  temp2=0.0;
static double  ro=1.0;
static double  cp=1.0;
static double  k=1.0;
static double  safe=0.7;
static double  sfact=1.0;
static double  dtmax=1.0;
static double  fac1 = 15.0/12.0;
static double  fac2 = 1.0/12.0;
...
//****************************************************************************
{
  ...
  MPI_Errhandler errh;
  MPI_Comm parent;
  int fsdo, nx, nt, fn, tag;
  int rc;
  double dt;
  double dtlast, rtime,  pos, source, tr;
//  ofstream t_file;
...
//
  ...
  MPI_Init ( &argc, &argv );
    ...
    my_chp = (double*)malloc(Tsize*sizeof(double));
    //my_chp = new double [Tsize];
    //buddy_chp = new double [Tsize];
    buddy_chp = (double*)malloc(Tsize*sizeof(double));
    F = (double*)malloc((npp+1)*sizeof(double));
    //F = new double [npp+1];
    //S = new double [npp];
    S = (double*)malloc(npp*sizeof(double));
    rhs = (double*)malloc(npp*sizeof(double));
...
//    fac2 = 1.0/12.0;
	...
	//	MPI_Barrier(MPI_COMM_WORLD);
	    ...
	    if (rank > 0 && rank < np)
	      {
		tag = 1;
		MPI_Send (&T[2], 2, MPI_DOUBLE, rank-1, tag, world);
	      }
	    ...
	    if (rank < np-1)
	      {
		tag = 1;
		MPI_Recv (&T[npp+2], 2, MPI_DOUBLE, rank+1, tag, world, &status);
	      }
	    ...
	    if (rank < np-1)
	      {
		tag = 1;
		MPI_Send (&T[npp], 2, MPI_DOUBLE, rank+1, tag, world);
	      }
	    ...
	    if (rank > 0 && rank < np)
	      {
		tag = 1;
		MPI_Recv (&T[0], 2, MPI_DOUBLE, rank-1, tag, world, &status);
	      }	    
	    ...
	    //cout << x0+(2+rn)*h<<"\n";
		  ...
		  //S[i] = 0.01;
		    //                    S[i] = sfact*sin(-(pos-time)*(pos-time));
                    S[i] = sfact*exp(-(pos-rtime)*(pos-rtime));
                }
	    //cout << S[0] << "\t" << S[1] << "\t" << S[2] << "\n";
	    //Calculate RHS
            for (int i=0; i<npp; i=i+1)
...
//	    //Output results
	    MPI_Barrier(world);
//	    char fname[10];
//	    sprintf(fname,"%d.%s",100000*(rank+1)+iteration,"txt");
//	    t_file.open (fname);
